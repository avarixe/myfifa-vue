<script>
  ...
  export default {
    ...
    setup (props) {
      ...
      const { playerDevelopmentStats, playerPerformanceStats, season } = toRefs(props)
      const store = useStore()
      const rows = computed(() => playerDevelopmentStats.value.map(stats => {
        const player = store.$db().model('Player').find(stats.playerId)
        ...
        const matchStats = playerPerformanceStats.value.reduce((totalStats, data) => {
        ...
        return {
          ...
          ...matchStats,
          link: player.link,
          age: player.age - (currentSeason.value - season.value),
          endOvr,
          endValue,
          ovrChange,
          valueChange,
