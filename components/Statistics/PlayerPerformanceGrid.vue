<script>
  import { ref, reactive, toRef, computed, useStore } from '@nuxtjs/composition-api'
    ...
    setup (props) {
      ...
      const rows = computed(() => players.value.filter(player => {
        switch (playerFilter.value) {
          ...
          case 4: // Loaned
          case 5: // Pending
            return player.status === currentPlayerFilter.value.text
        }
      }).map(player => {
        const playerStats = []
        const matchStats = (statsByPlayerId.value[player.id] || []).reduce(
          (totalStats, data) => {
            if (
      ...
      const headers = computed(() => [
        { text: 'Name', value: 'name', width: 200, class: expanded.value.length ? '' : 'stick-left' },
      ...
      >
        ...
        <template #item="{ item, expand, isExpanded }">
          <tr @click="expand(!isExpanded)">
            <td :class="{ 'stick-left': !expanded.value.length }">
              <v-btn
                small
                text
                color="info"
                class="text-capitalize"
                @click.stop="$router.push(`/teams/${team.id}/players/${item.id}`)"
              >
                {{ item.name }}
              </v-btn>
            </td>
